...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/meetings/meetingsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureMeetingsCols';
...
const TableSampleMeetings = ({
  ...
  useEffect(() => {
    if (meetingsNotify.showNotification) {
      notify(meetingsNotify.typeNotification, meetingsNotify.textNotification);
    }
  }, [meetingsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/meetings/meetings-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/meetings/meetings-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          ...
          pathEdit={`/meetings/meetings-edit/?id=`}
          pathView={`/meetings/meetings-view/?id=`}
...
export default TableSampleMeetings;
