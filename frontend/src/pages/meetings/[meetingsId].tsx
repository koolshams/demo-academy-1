...
const EditMeetings = () => {
  ...
  const { meetingsId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: meetingsId }));
  }, [meetingsId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: meetingsId, data }));
...
EditMeetings.getLayout = function getLayout(page: ReactElement) {
  return (
